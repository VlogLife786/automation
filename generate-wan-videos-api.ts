...
import { AvailableCreditsApiResponse, ExecutionRequestModel, GenericApiResponse, GetPolicyApiResponse, LoginRequestModel, StartVideoGenerationResponse, TaskResultByIdResponse, UploadedFileOssUrlResponse } from "./wan-video-object-models";
...
export async function GenerateWANAiVideosByApi(requestModel: ExecutionRequestModel) {
    ...
    try {
        ...
        if (requestModel.startImageName && requestModel.startImageName != null && requestModel.startImageName != "") {
            ...
            let ossResponse: UploadedFileOssUrlResponse = await GenerateOssResponse(policyResponse.data.key);
...
async function GenerateOssResponse(key: string) {
    try {
        let data = JSON.stringify({
            "key": key,
            "taskType": ""
