 ...
 */
async function validateWANOtp(wanPage, otp, tempMail) {
 ...
 */
async function yopmail(emailId) {
...
async function generateTempMail() {
    ...
    return await tempMail.$eval('#i-email', el => el.value.trim());
}
/**
 ...
 */
async function wanAIRegistration(emailId, name, password) {
    // Launch browser
 ...
 */
async function focusOnPage(pageName) {
 ...
 * @returns Page object
 */
async function getPageFromOpenedByPageName(pageName) {
    let pages = await browser.pages();
    for (const page of pages) {
        let currentPageName = await page.title();
        if (currentPageName.includes(pageName)) {
 ...
 * @returns OTP
 */
async function getWanOTPFromYopmail() {
    ...
    try {
        ...
        let divText = await frame.$eval('#mail', el => el.textContent?.trim() ?? null);
 ...
 */
async function getEmailFromTempMailSo() {
 ...
 * @returns OTP
 */
async function getWanOTPFromTempMailSo() {
 ...
 */
async function openNewBrowser(instanceType) {
    return instanceType ?
