...
import { AvailableCreditsApiResponse, GenericApiResponse, LoginRequestModel } from "./wan-video-object-models.js";
...
export async function claimDailyCredits() {
    let creds: any = await getRestResponse(`${ApiURLs.USER_DETAILS_GOOGLE_SHEET}?action=getNotCreditClaimedUser`);
    ...
    try {
        let loginResponse: GenericApiResponse = await loginToWanAIForCredits({
        ...
        let availableCredits: AvailableCreditsApiResponse = await getAvailableCreditsForDailyCheck();
...
export async function loginToWanAIForCredits(loginrequest: LoginRequestModel): Promise<GenericApiResponse> {
    try {
...
export async function getAvailableCreditsForDailyCheck(): Promise<AvailableCreditsApiResponse> {
    try {
        let data = JSON.stringify({});
...
export async function extractAuthHeaderForDailyCheck(headers: string[], cookies: string[]) {
