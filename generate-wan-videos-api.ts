...
import { AvailableCreditsApiResponse, ExecutionRequestModel, GenericApiResponse, GetPolicyApiResponse, LoginRequestModel, RefrenceImageDetails, StartVideoGenerationResponse, TaskResultByIdResponse, UploadedAudioCdnResponse, UploadedImageFileOssUrlResponse } from "./wan-video-object-models.js";
import { globalVars, replaceString, sleep } from "./utility.js";
...
export async function GenerateWANAiVideosByApi(requestModel: ExecutionRequestModel) {
    let executionSteps: string[] = [];
    console.log("Received the request of execution...");

    try {
        let loginResponse: GenericApiResponse = await loginToWanAI({
        ...
        if (requestModel.refrenceImageList && requestModel.refrenceImageList.length > 0) {
            ...
            if (requestModel.startImageName && requestModel.startImageName != null && requestModel.startImageName != "") {
                //Image upload
                let ossUploadedImageResponse: UploadedImageFileOssUrlResponse = await uploadImageToWanAiServer(requestModel, executionSteps);
                await sleep(1000);
                ...
                let startImageDetaiils: RefrenceImageDetails = {
            ...
            if (requestModel.audioFileName && requestModel.audioFileName != null && requestModel.audioFileName != "") {
                ...
                let cdnOssAudioResponse: UploadedAudioCdnResponse = await generateBatchCdnForAudio(policyResponse.data.key);
                ...
                let ossResponse: UploadedImageFileOssUrlResponse = await uploadImageToWanAiServer(requestModel, executionSteps);
        ...
        if (videoGenerationResponse.success && videoGenerationResponse.data) {
            for (let index = 1; index <= 300; index++) {
                let taskResultResponse: TaskResultByIdResponse = await getTaskDetailsById(videoGenerationResponse.data);
...
async function generateVideoForImageAndAudio(textPrompt: string, uploadedImageUrl: string, uploadedAudioUrl: string): Promise<StartVideoGenerationResponse> {
    try {
        ...
        let config = {
            ...
            headers: {
                ...
                'sec-ch-ua': '"Google Chrome";v="' + globalVars.chromeVersion + '", "Not.A/Brand";v="8", "Chromium";v="' + globalVars.chromeVersion + '"',
                'sec-ch-ua-mobile': '?0',
...
async function uploadImageToWanAiServer(requestModel: ExecutionRequestModel, executionSteps: string[]) {
    let filetype = await fileTypeFromFile(Configs.UPLOADED_IMAGE_DIR + requestModel.startImageName);
    let policyResponse: GetPolicyApiResponse = await getPolicyForFile(requestModel.startImageName + "." + (filetype?.ext ?? "png"));
    ...
    let ossResponse: UploadedImageFileOssUrlResponse = await GenerateImageFileOssResponse(policyResponse.data.key);
...
async function uploadRefrenceImageToWanAiServer(requestModel: ExecutionRequestModel, executionSteps: string[]) {
    for (const image of requestModel.refrenceImageList) {
        ...
        let policyResponse: GetPolicyApiResponse = await getPolicyForFile(image.imageName + "." + (filetype?.ext ?? "png"));
...
async function generateVideoByImage(baseImageUrl: string, textPrompt: string) {
    try {
        let data = JSON.stringify({
            "deductMode": "credit_mode",
            "taskType": "image_to_video",
...
async function GenerateImageFileOssResponse(key: string) {
    try {
        let data = JSON.stringify({
            "key": key,
            "taskType": ""
...
async function uploadFileToServer(fileNameWithPath: string, policyResponse: GetPolicyApiResponse) {
    try {
...
export async function getPolicyForFile(fileName: string): Promise<GetPolicyApiResponse> {
    try {
        let data = JSON.stringify({
            "fileName": fileName,
            "taskType": ""
...
export async function loginToWanAI(loginrequest: LoginRequestModel): Promise<GenericApiResponse> {
    try {
...
export async function startVideoGeneration(textPrompt: string): Promise<StartVideoGenerationResponse> {
    try {
        let data = JSON.stringify({
            "deductMode": "credit_mode",
...
export async function getTaskDetailsById(taskId: string) {
    try {
        let data = JSON.stringify({
            "taskId": taskId
        });
...
async function generateBatchCdnForAudio(key: string): Promise<UploadedAudioCdnResponse> {
    try {
        let data = JSON.stringify({
            "ossPathList": [
                key
...
export async function extractAuthHeader(headers: string[], cookies: string[]) {
...
export async function generateVideosForRefrenceImage(requestModel: ExecutionRequestModel) {
    let requestJson: any = {
        "deductMode": "credit_mode",
    ...
    let config = {
        ...
        headers: {
            ...
            'sec-ch-ua': '"Chromium";v="' + globalVars.chromeVersion + '", "Google Chrome";v="' + globalVars.chromeVersion + '", "Not/A)Brand";v="99"',
            'sec-ch-ua-mobile': '?0',
            'sec-ch-ua-platform': '"Windows"',
            'sec-fetch-dest': 'empty',
            'sec-fetch-mode': 'cors',
...
export async function generateRefrenceVideoWithFirstFrame(requestModel: ExecutionRequestModel, startImageDetails: RefrenceImageDetails) {
    let requestJson: any = {
        "deductMode": "credit_mode",
        "taskType": "cast_to_video",
        "taskInput": {
