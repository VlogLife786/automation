...
export async function searchOnGemini(textPrompt: string, imagePaths: string[] = [], retries = 5, closeBrowserAfterDone: boolean = true): Promise<string | undefined> {
    ...
    try {
        ...
        await page.waitForSelector('[data-placeholder="Ask Gemini"]', { timeout: 60000 });
        ...
        await writeTextInTextbox(page, '[data-placeholder="Ask Gemini"]', textPrompt);
        ...
        if (retries > 0) {
            ...
            return await searchOnGemini(textPrompt, imagePaths, retries - 1, closeBrowserAfterDone);
        } else {
...
async function checkResponseCompleted(page: Page, retries = 7) {
    let responseFetchInProgress = await page.$$('[fonticon="stop"]');
