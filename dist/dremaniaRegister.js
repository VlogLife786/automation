...
export async function registerToDremaniaAi() {
    ...
    try {
        ...
        await page.waitForFunction(() => {
            let dateOfBirthParagraph = document.querySelector(".lv_new_sign_in_panel_wide-birthday-subtitle");
        ...
        await page.evaluate(async () => {
            let monthElements = Array.from(document.querySelectorAll("#lv-select-popup-0 li"));
            let randomMonthNumber = Math.round(Math.random() * (monthElements.length - 1) + 1);
            monthElements[randomMonthNumber].click();
        });
        // Enter date of birth date 
        await page.click(".lv-select-view-value.lv-select-view-value-mirror");
        await page.evaluate(async () => {
            let dateOfBirthDateElemennts = Array.from(document.querySelectorAll("#lv-select-popup-1 li"));
            let randomDateNumber = Math.round(Math.random() * (dateOfBirthDateElemennts.length - 1) + 1);
 ...
 */
async function getRandomBirthYear(minAge = 18, maxAge = 40) {
 ...
 */
export async function getRandomNumber(min = 1, max) {
