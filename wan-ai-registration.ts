...
export async function startProcessOfAccountCreation() {
    ...
    try {
        let tempMail: string = await getEmailFromTempMailSo(globalVars.globalBrowser);
        ...
        let wanPage: Page = await getPageFromOpenedPages(globalVars.globalBrowser, PageNames.WAN_AI);
        try {
